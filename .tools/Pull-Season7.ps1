...
$baseApiUrl = "https://hearthstone.blizzard.com/en-us/api/community/leaderboardsData?region=$region&leaderboardId=battlegroundsduo&page="
    ...
    if ($response.PSObject.Properties.Name -contains "leaderboard") {
        ...
        if ($leaderboard.PSObject.Properties.Name -contains "rows") {
            ...
            foreach ($row in $rows) {
                ...
                Invoke-RestMethod -Uri "https://hearthstone-bg-leaderboard.gamerz-bg.com/api/$season/$type/$regionApi/record" -Headers $headers -Method POST -Body "rank=$rank&accountid=$encodedAccountId&rating=$rating"
            }
