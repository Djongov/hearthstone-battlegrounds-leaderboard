...
if ($PSVersionTable.PSVersion.Major -lt 7) {
    Write-Host "PowerShell version is less than 7. Exiting script."
    exit 1
}
        ...
        if ($leaderboard.PSObject.Properties.Name -contains "rows") {
            ...
            foreach ($row in $rows) {
                ...
                try {
                    ...
                    $callApi = Invoke-RestMethod -Uri "https://hearthstone-bg-leaderboard.gamerz-bg.com/api/$season/$type/$regionApi/record" -Headers $headers -Method POST -Body "rank=$rank&accountid=$encodedAccountId&rating=$rating" -SkipCertificateCheck -SkipHttpErrorCheck -UserAgent "Djo-Automation-Azure"
